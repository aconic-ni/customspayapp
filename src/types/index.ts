...
export interface SolicitudData {
  id: string;
  reference?: string; // Added: Per-solicitud reference
...
export interface SolicitudRecord {
  examNe: string;
  examReference: string | null; // This will now store the per-solicitud reference
  examManager: string;
...
// For exporting, InitialDataContext reference is removed. Each solicitud has its own reference.
export interface ExportableSolicitudContextData extends Omit<InitialDataContext, 'date'> {
  date?: Date | FirestoreTimestamp | null; 
  solicitudes?: SolicitudData[] | null; // SolicitudData now contains 'reference'
